...
def speculative_sampling(target_model, draft_model, tokenizer, prompt_ids, max_new_tokens,
                         ...
                         accept_mode='prob', target_context_len=None, draft_context_len=None,
                         debug=False):
    """
    Speculative Sampling (Draft-Verification-Correction) with configurable acceptance
    and different context windows for target/draft.

    accept_mode:
      'prob'      - original Monte Carlo rejection sampling:
                    accept with prob min(1, Pt(x)/Pd(x))
    ...
    while idx.shape[1] < T:
        # --- 1. Draft Generation ---
        draft_tokens = []
        draft_distributions = []  # store P_d
        curr_idx = idx.clone().to(device_draft)

        for _ in range(gamma):
            if curr_idx.shape[1] >= T:
                break

            if curr_idx.size(1) <= draft_context_len:
                idx_cond = curr_idx
            else:
                idx_cond = curr_idx[:, -draft_context_len:]

            with torch.no_grad():
                logits = draft_model(idx_cond)
            ...
            filtered_logits = top_k_top_p_filter(next_token_logits.clone(), top_k=top_k, top_p=top_p)
            ...
            # move everything to target device for consistency
            draft_tokens.append(next_token.to(device_target))
            draft_distributions.append(probs.to(device_target))
        ...
        relevant_logits = target_logits[:, -(num_draft + 1):, :]
        ...
        for i in range(num_draft):
            ...
            p_d = draft_distributions[i]  # [1, vocab]
            ...
            if accept_mode == 'prob':
                # Original: accept with probability min(1, Pt/Pd)
                r = torch.rand(1).item()
                ...
                if r < accept_prob:
                    ...
                    accepted_count += 1
                    if debug:
                        print(f"[prob] Step {i}: Accepted {token_id} "
                              f"(Pd={p_d_token:.4e}, Pt={p_t_token:.4e}, r={r:.4f}, acc_p={accept_prob:.4f})")
                else:
                    all_accepted = False
                    if debug:
                        print(f"[prob] Step {i}: Rejected {token_id} "
                              f"(Pd={p_d_token:.4e}, Pt={p_t_token:.4e}, r={r:.4f}, acc_p={accept_prob:.4f})")
                ...
                # Accept iff Pt(x) > Pd(x)
                if p_t_token > p_d_token:
                    idx = torch.cat((idx, draft_tokens[i]), dim=1)
                    accepted_count += 1
                    if debug:
                        print(f"[pt_gt_pd] Step {i}: Accepted {token_id} "
                              f"(Pd={p_d_token:.4e}, Pt={p_t_token:.4e})")
                else:
                    all_accepted = False
                    if debug:
                        print(f"[pt_gt_pd] Step {i}: Rejected {token_id} "
                              f"(Pd={p_d_token:.4e}, Pt={p_t_token:.4e})")
                ...
                # Deterministic: accept only if draft token == target argmax
                ...
                if token_id == target_token_id:
                    ...
                    accepted_count += 1
                    if debug:
                        print(f"[match] Step {i}: Accepted {token_id}")
                else:
                    all_accepted = False
                    idx = torch.cat((idx, target_argmax), dim=1)
                    if debug:
                        print(f"[match] Step {i}: Rejected draft {token_id}, "
                              f"using target {target_token_id}")
                    break
        ...
        if all_accepted:
            ...
            if bonus_token.item() not in (tokenizer.unk_id, tokenizer.pad_id):
                ...
                if debug:
                    print(f"Bonus: {bonus_token.item()}")
...
def main():
    ...
    # hg38 CSV sampling
    parser.add_argument('--hg_csv', default='hg38_sequences.csv',
    ...
    ### NEW: control prompt size in tokens
    parser.add_argument(
        '--max_prompt_tokens',
        type=int,
        default=32,
        help='If >0, truncate encoded prompt to this many tokens (including prefix).'
    )
    ...
    ### NEW: truncate encoded prompt to max_prompt_tokens
    if args.max_prompt_tokens > 0 and prompt_ids.shape[0] > args.max_prompt_tokens:
        prompt_ids = prompt_ids[:args.max_prompt_tokens]
        print(f"Truncated prompt to {args.max_prompt_tokens} tokens.")
    else:
        print(f"Prompt uses {prompt_ids.shape[0]} tokens (no truncation).")
    ...
    for i in range(args.num_samples):
        idx, duration = generate_baseline(
            ...
            args.temperature, args.topk, args.topp,
        ...
        print(f"Sample {i+1}: {n_new} tokens in {duration:.2f}s ({n_new/duration:.2f} tps)")
    ...
    for i in range(args.num_samples):
        idx, duration, acc_rate = speculative_sampling(
            ...
            args.L, args.temperature, args.topk, args.topp,
    ...
    target_tps = get_stats('target')
    draft_tps = get_stats('draft')
    specdec_tps = get_stats('specdec')
    avg_acc_rate = sum(metrics['specdec']['acceptance_rate']) / len(metrics['specdec']['acceptance_rate'])

    print(f"Target Model TPS: {target_tps:.2f}")
    print(f"Draft Model TPS:  {draft_tps:.2f}")
    print(f"SpecDec TPS:      {specdec_tps:.2f}")
    print(f"Speedup vs Target: {specdec_tps/target_tps:.2f}x" if target_tps > 0 else "Speedup vs Target: N/A")
    print(f"Mean Acceptance Rate: {avg_acc_rate:.2f}")
    print(f"Mean Accepted Prefix Length: {avg_acc_rate * args.L:.2f}")
